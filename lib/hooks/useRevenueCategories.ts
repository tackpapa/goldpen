...
interface UseRevenueCategoriesOptions {
  orgSlug?: string
}
...
export function useRevenueCategories(options: UseRevenueCategoriesOptions = {}) {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/settings/revenue-categories?${params}`, {
        credentials: 'include'
      })
      ...
      console.error('[useRevenueCategories] Error:', err)
    } finally {
      setLoading(false)
    }
  }, [options.orgSlug])
  ...
  const addCategory = async (data: RevenueCategoryInput): Promise<RevenueCategory | null> => {
    try {
      const res = await fetch('/api/settings/revenue-categories', {
        method: 'POST',
  ...
  const updateCategory = async (id: string, data: Partial<RevenueCategory>): Promise<boolean> => {
    try {
      const res = await fetch('/api/settings/revenue-categories', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const deleteCategory = async (id: string): Promise<boolean> => {
    try {
      const res = await fetch(`/api/settings/revenue-categories?id=${id}`, {
        method: 'DELETE',
        credentials: 'include'
      })
